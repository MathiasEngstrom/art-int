diagnose <- function(network, cases) {
  ...
  P <- function(sample, known, probs = network) {
    ...
    probability <- P_pn[pn] * P_tb[tb] * P_lc[lc, sm] * P_br[br, sm] *
      P_te(te)[pn] * P_vtb[vtb] * P_sm[sm] * P_xr[xr, pn, tb, lc] *
      P_dy[dy, lc, br]

    return(probability)
  }
  
  #########3 Candidate function for binary attributes #####################
