makeMoves <- function (moveInfo, readings, positions, edges, probs) {
  ...
  # Backpacker 2 has died, croc is here with prob 1  
    ...
    # Croc can't be on the same nodes as the backpackers
    ...
    if(is.null(moveInfo$moves)) {
      ...
      if(moveInfo$moves[1] == 0 ) {
        s0[moveInfo$moves[1]] <- 0
      }
      
      if(moveInfo$moves[2] == 0) {
        s0[moveInfo$moves[2]] <- 0
      }
    }
  ...
  without_highest <- s1[-highest_prob]
  second_highest <- which.max(without_highest)
  
  move <- moveInfo$mem$path_matrix[ranger, highest_prob]
  move2 <- moveInfo$mem$path_matrix[ranger, second_highest]
